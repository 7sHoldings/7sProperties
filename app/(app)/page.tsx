...
export default async function DashboardPage() {
  ...
  const [propsRes, unitsRes, leasesRes, paymentsRes, expensesRes] = await Promise.all([
    ...
    supabase
      .from("leases")
      .select("id, monthly_rent, status, tenant_id, tenants(full_name)")
      .eq("status", "active"),
    supabase
      .from("payments")
      .select(
        "id, amount, payment_date, lease_id, leases(tenants(full_name))"
      )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div className="bg-white border border-stone-200 rounded-xl p-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <Link href="/payments" className="text-xs text-teal-700">
              View all
            </Link>
          </div>
          {monthPayments.length === 0 ? (
            ...
            <ul className="text-sm">
              {monthPayments.slice(0, 5).map((p: any) => (
                ...
                >
                  <span className="truncate">{p.leases?.tenants?.full_name || "—"}</span>
